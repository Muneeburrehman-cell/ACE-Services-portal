...
async function run() {
  ...
  console.log('--- Starting Complete Flow Test Against Running API ---');

  // 1. BD Agent Login
  console.log('1. Logging in as BD Agent...');
  ...
  console.log('   ✓ BD Agent logged in. Token acquired.');

  // 2. Submit New Project with new fields
  console.log('2. Submitting New Project with Company, Contact Person, Salesperson, Decided Price ($2,850)...');
  const createRes = await fetch(`${API_URL}/projects`, {
    ...
    body: JSON.stringify({
      clientCompanyName: 'Apex Structural Builders',
      ...
      scopeDescription: 'Full CSI Division 3 Concrete and Division 5 Metals takeoff for 3-story commercial complex.',
    }),
  });
  ...
  console.log(`     - Company: ${project.clientCompanyName}`);
  console.log(`     - Contact: ${project.clientContactPerson}`);
  ...
  console.log(`   ✓ Updated to Proposal: ${propData.status}`);
  ...
  console.log(`   ✓ Updated to Follow-Up: ${fuData.status} (Due: ${fuData.followUpDate})`);
  ...
  console.log(`   ✓ Updated to Approved: ${appData.status}`);

  // 4. Admin Login & Pipeline Management
  console.log('4. Logging in as Administrator...');
  ...
  console.log(`   ✓ Found engineer: ${targetEng.fullName} (${targetEng.email}, Role: ${targetEng.role})`);

  // 4b. Assign Project to Engineer
  console.log('5. Assigning project to Estimation Engineer...');
  ...
  console.log(`   ✓ Base Price: $${feeData.decidedPrice}`);
  console.log(`   ✓ Merchant Fee: ${feeData.merchantFeePercent}% ($${feeData.merchantFeeAmount})`);
  ...
  console.log('   ✓ Engineer logged in.');
  ...
  console.log(`   ✓ RFI created: "${rfiData.title}" (Status: ${rfiData.status})`);
  ...
  // 10. Admin Forwarding RFI to Client Email
  console.log('10. Admin forwarding RFI to Client Email via Resend...');
  const fwdRes = await fetch(`${API_URL}/projects/${project.id}/rfis/${rfiData.id}/forward-client`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${adminToken}` },
  });
  const fwdData = await fwdRes.json();
  console.log(`   ✓ Forwarded to: ${fwdData.forwardedTo}`);

  // 11. Final verification of project state
  console.log('11. Verifying full project record...');
